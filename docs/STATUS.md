...
- **Foundation: Shared components** — `components/shared/DataTable.tsx` (generic TanStack Table wrapper), `components/shared/PageHeader.tsx`, `components/shared/InlineTextareaCell.tsx`, `components/shared/InlineStageCell.tsx` (now shows a confirmation dialog when re-staging away from a Won/Lost opportunity, per `docs/ARCHITECTURE.md`; dialog's Cancel/Confirm buttons disabled while the mutation is in flight to prevent a double-submit), `components/shared/Sidebar.tsx`, `components/shared/QueryProvider.tsx`

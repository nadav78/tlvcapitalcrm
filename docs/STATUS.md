...
- **Foundation: Shared components** — `components/shared/DataTable.tsx` (generic TanStack Table wrapper), `components/shared/PageHeader.tsx`, `components/shared/InlineTextareaCell.tsx`, `components/shared/InlineStageCell.tsx` (shows a confirmation dialog when re-staging away from a Won/Lost opportunity, per `docs/ARCHITECTURE.md`; built with shadcn `AlertDialog` — not a hand-rolled overlay — with Cancel/Confirm disabled while the mutation is in flight), `components/shared/Sidebar.tsx`, `components/shared/QueryProvider.tsx`
- **shadcn components installed** — `button`, `alert-dialog`

...
- **PR #8** (`fix/last-activity-cell-purity`, merged directly — mechanical, no observable behavior change, low-risk per CLAUDE.md's merge-without-review criteria) fixed a pre-existing `npm run lint` error (not a warning) in `features/opportunities/columns.tsx`'s `LastActivityCell`: it called `Date.now()` directly during render, which `react-hooks/purity` (part of the React Compiler ESLint config already enabled in this project) flags as impure. Fixed by capturing `now` once via a lazy `useState(() => Date.now())` initializer instead. Documented as a reusable pattern in ARCHITECTURE.md's new "Render Purity (React Compiler)" section under Design Principles, since the same "days ago" pattern is specified for the not-yet-built Contacts list (`last_activity_at`, red if > 30 days) and will very likely hit the same lint error when built.

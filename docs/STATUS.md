...
- **Opportunities: New Opportunity page** — `app/(app)/opportunities/new/page.tsx` (Server Component; redirects `sector_manager` to `/opportunities` since they cannot create opportunities per PRODUCT.md §6 — guards direct URL navigation, not just the hidden button on the list page) + `features/opportunities/components/OpportunityRegisterForm.tsx` (full page per the Form Presentation table; RHF + `opportunityRegisterSchema`). RSM is locked to self and hidden from the form entirely for the `rsm` role; Admin gets a visible RSM picker, and `region_id` is derived from the selected RSM rather than independently editable (avoids an inconsistent RSM/region pair, same spirit as the RSM Reassignment Cascade pattern). Stage silently defaults to the default stage (`pipeline_stages.is_default`, set once `usePipelineStages()` resolves, submit disabled until then). Registration date defaults to today via a lazy `useState` initializer per the Render Purity rule. On success: toast + redirect to `/opportunities` (not the opportunity's own detail page, since that doesn't exist yet).
  ...
  - **`/review 9` (independent fresh-session pass) found 5 issues; all fixed on this branch:**
    ...
    - **Fixed — the Admin RSM `<Select>` was the only one in the form not wired through `Controller`/`value`,** inconsistent with the `value={field.value ?? null}` convention used by every other Select here (and documented in ARCHITECTURE.md's Base UI gotchas). Wrapped in `Controller`; also added `{ shouldValidate: true }` to the `rsm_id`/`region_id` `setValue` calls in `handleRsmChange` so a prior validation error clears immediately on selection instead of lingering until the next submit.
